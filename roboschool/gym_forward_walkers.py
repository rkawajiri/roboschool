...
class RoboschoolHopper(RoboschoolForwardWalkersBase):
    foot_list = ["foot"]
    def __init__(self):
        RoboschoolForwardWalkersBase.__init__(self, "hopper.xml", "torso", action_dim=3, obs_dim=36, power=0.75)
...
class RoboschoolWalker2d(RoboschoolForwardWalkersBase):
    ...
    def __init__(self):
        RoboschoolForwardWalkersBase.__init__(self, "walker2d.xml", "torso", action_dim=6, obs_dim=64, power=0.40)
    def alive_bonus(self, z, pitch):
        return +1 if z > 0.8 and abs(pitch) < 1.0 else -1
    def robot_specific_reset(self):
